...
import { getAllFoodWithStockAndActive, getRelationOfFood } from '../service_db/DBQuerys.jsx'; // Importar las funciones de la DB

export default function FoodPicker2({ data, goTo }) {
  const [filters, setFilters] = useState({vegetarian: false, vegan: false, celiac: false, type: 'comida' });
    ...
    const loadFoods = async () => {
      try {
        ...
        const foodsWithRestrictions = foodData.map(food => {
          const foodRelations = relations.filter(relation => relation.id_food === food.id);
          const foodRestrictions = foodRelations.map(rel => rel.code_restriction);
          return {
            ...food,
        ...
        const selectedFoodsFromDB = foodsWithRestrictions.filter(food => selectedFoodIDs.includes(food.id));
  ...
  function toggleSelectedFood(food) {
    if (selectedFoods.includes(food)) {
      setSelectedFoods(selectedFoods.filter(selectedFood => selectedFood.id !== food.id));
    } else {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.navContainer}>
        ...
        <TabIcon
          text='Comida'
          svg={<Food fill={filters.type === 'comida' ? 'white' : undefined} />}
          onPress={() => setFilters({ ...filters, type: 'comida' })}
          style={filters.type === 'comida' ? { backgroundColor: 'black', borderRadius: 15 } : undefined}
          selected={filters.type === 'comida'}
        />
        <TabIcon
          text='Bebida'
          svg={<Drinks fill={filters.type === 'bebida' ? 'white' : undefined} />}
          onPress={() => setFilters({ ...filters, type: 'bebida' })}
          style={filters.type === 'bebida' ? { backgroundColor: 'black', borderRadius: 15 } : undefined}
          selected={filters.type === 'bebida'}
        />
        <TabIcon
          text='Postre'
          svg={<Dessert fill={filters.type === 'postre' ? 'white' : undefined} />}
          onPress={() => setFilters({ ...filters, type: 'postre' })}
          style={filters.type === 'postre' ? { backgroundColor: 'black', borderRadius: 15 } : undefined}
          ...
          <BackButton
            ...
            onPress={() => {
              if (selectedFoods.length === 0) {
                Toast.show({ type: 'info', text1: 'Primero debes seleccionar algún plato' });
                return;
              }
      ...
      <MenuList alignTop={true}>
        <View style={{ alignItems: 'center' }}>
          <View style={styles.filterContainer}>
            <TabIcon
              svg={<Vegetarian fill={filters.vegetarian ? 'white' : undefined} />}
              onPress={() => setFilters({ ...filters, vegetarian: !filters.vegetarian })}
              style={filters.vegetarian ? { backgroundColor: 'green', borderRadius: 15 } : undefined}
            />
            <TabIcon
              svg={<Vegan fill={filters.vegan ? 'white' : undefined} />}
              onPress={() => setFilters({ ...filters, vegan: !filters.vegan })}
              style={filters.vegan ? { backgroundColor: 'green', borderRadius: 15 } : undefined}
            />
            <TabIcon
              svg={<Celiac fill={filters.celiac ? 'white' : undefined} />}
              onPress={() => setFilters({ ...filters, celiac: !filters.celiac })}
              style={filters.celiac ? { backgroundColor: 'green', borderRadius: 15 } : undefined}
            />
          </View>
        </View>
        {foodList.map((food, index) => (
          <FoodItem
            selected={selectedFoods.some(selectedFood => selectedFood.id === food.id)}
            key={index}
            title={food.name}
...
function matchesFilters(food, filters) {
  ...
  if (filters.type === 'comida') {
    ...
    if (filters.vegetarian && !food.restrictions.includes(2) && !food.restrictions.includes(1)) return false; // 2: vegetariano, 1: vegano
